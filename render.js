...
import { renderMonthGrid, renderDayDetail, calNav } from "./calendar.js";
...
function renderTabs() {
  return `<div class="tabs" role="tablist">
    <button class="tab" role="tab" type="button" data-view="cal"  aria-selected="${state.view === "cal" || state.view === "today"}">${t("cal")}</button>
...
export function render() {
  ...
  if (state.view === "done") {
    ...
    views.innerHTML = renderTabs() + renderWeek(now);
  } else {
    // default: calendar view (cal or today)
    views.innerHTML = renderTabs() +
      `<div class="cal-wrap">${renderMonthGrid(render)}</div>` +
      `<div class="day-detail">${renderDayDetail(now)}</div>`;
  }
}
